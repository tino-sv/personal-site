   ...
   <div className="flex flex-col min-h-screen p-8 max-w-2xl mx-auto">
      ...
      <main className="flex-grow space-y-12">
          <section className="space-y-4">
            ...
            <p className="text-sm text-green-400/70">thoughts on code and other web things</p>
          </section>

          <section className="space-y-6">
            {blogPosts.map((post) => (
              <article key={post.id} className="space-y-3 border border-green-400/20 p-4 rounded-lg">
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs text-green-400/50">{post.readingTime}</span>
                </div>
                <time dateTime={post.date} className="text-xs text-green-400/50 block">
                ...
                <p className="text-sm text-green-400/70">{post.excerpt}</p>
                <div className="pt-2">
                  <Link 
                    href={`/blog/${post.id}`}
